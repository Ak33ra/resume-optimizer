...
2. **`CRITERIA.md`** — how resumes are scored (fatal gate + weighted rubric +
   the blind verifier panel + KEEP/REVERT rule).
...
5. **`TOOLS.md`** — exact orchestration and verification commands
   (`scripts/round.py`, gates, panel, git protocol).
...
- **Blind verification.** Use paired blind scoring with at least three
  judgments. Record reviewer families; correlated panels use a wider margin.
